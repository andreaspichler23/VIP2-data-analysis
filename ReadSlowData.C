...
int MakeSlowTree( TString slow_file, TString rootfilename){
    ...
    while(fgets(fline,512,slowfile)){
     ...
     sscanf( fline, "%2d/%2d/%4d %2d:%2d:%2d %s %s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf \n",

     &slow_s.month, &slow_s.day, &slow_s.year, &slow_s.hour, &slow_s.min, &slow_s.sec, &slow_s.AmOrPm, &slow_s.ticks, &slow_s.room, &slow_s.ar_target, &slow_s.pcb_a, &slow_s.pcb_b, &slow_s.sdd1, &slow_s.argon_up, &slow_s.argon_down, &slow_s.sdd2, &slow_s.copper_int, &slow_s.copper_ext1, &slow_s.cooling_pad, &slow_s.vacuum_pres, &slow_s.current, &slow_s.argon_pres, &slow_s.voltage, &slow_s.heater_percent, &slow_s.ar_gas);

   // cout << slow_s.year << " " << slow_s.room << " " << slow_s.min << " " << slow_s.ar_gas << endl;
