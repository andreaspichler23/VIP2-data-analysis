...
void MakeTree( TString filelist, TString rootfilename ) 
{
    ...
    tr->Branch("evid2",  &evt.evid2,  "Event number tdc/S");
    ...
    TH1F* hp[16];       // PADC
    TH1F* hq[NSiPM];    // QDC
    TH1F* hqc[NSiPM];   // QDC with tot cut

    TH2F* hqt[NSiPM];   // QDC - ToT correlation

    TH2F* hpos;         // Hit position

    for( Int_t i=0; i < V1190_TDC_CH; i++ ){
        ht[i] = new TH1F(Form("ht[%d]", i), "Timing spectra", 128, -200.5, 1847.5);
    }
...
Int_t ReadData( TString datafilename, TString root_file )
{
    ...
    evt_r.evid = 0,   evt_r.evid1 = 0,  evt_r.evid2 = 0;
    ...
    */
    ...
    // first line is different fml, read in first line seperately --------------
    if ( fread(&dummyline, sizeof(dummyword), 28, datafile) != 28 )
  ...
  //  Int_t list [1] = {2};
    ...
    for( Int_t i = 0 ; i < 6 ; i++){
      ...
      // gPad->SetLogy();
       ...
       hsp -> SetMaximum(2000);
       hsp -> GetXaxis()->SetRangeUser(0,1000);
