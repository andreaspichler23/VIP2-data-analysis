...
#include  "common.h"
#include  "ReadFullDaqBinData_2.h" 
...
void MakeTree( TString filelist, TString rootfilename ) 
{
    ...
    tr->Branch("evid1",  &evt.evid1,  "Event number qdc/I");
    tr->Branch("evid2",  &evt.evid2,  "Event number padc/I");
    tr->Branch("evid3",  &evt.evid3,  "Event number tdc/I");
    tr->Branch("ut",     &evt.ut,     "Unix time tag/i");
    tr->Branch("rate",   &evt.rate,   "Trigger rate/S");
    tr->Branch("mul",    &evt.mul,    "Event multiplicity/S");
    tr->Branch("qdc",    evt.qdc,     "QDC Channel[32]/I");
    tr->Branch("tdc",    evt.tdc,     "TDC Channel[64]/I");
    tr->Branch("tot",    evt.tot,     "TimeOverThreshold[64]/I");
    tr->Branch("padc",   evt.padc,    "PeakADC[32]/I");
    //tr->Branch("layer",  evt.layer,   "Scinti layer/S");
    //tr->Branch("col",    evt.col,     "Scinti col/S");
    tr->Branch("trgid",     evt.trgid,      "Trigger type/I");
    tr->Branch("hr",        evt.hr,         "High Rate/I");
...
*/
    ...
    //    if ( fread(&evt_r.rate, sizeof(evt_r.rate), 1, datafile) != 1 ) break;
     ...
     //   if ( fread(&dummyword, sizeof(dummyword), 1, datafile) != 1 ) break;//-------------------------------hier evtl ändern wenn qdc anzahl sich ändert
        ...
        // evid from qdc  
         if ( fread(&evt_r.evid1, sizeof(evt_r.evid1), 1, datafile) != 1 ) break;     
  ...
  //      if ( fread(&dummyword, sizeof(dummyword), 1, datafile) != 1 ) break; // -1 coloumn only if disabled
	...
	if ( fread(&evt_r.evid3, sizeof(dummyword), 1, datafile) != 1 ) break;
        ...
        for( Int_t i=0; i < V1190_TDC_CH; i++){
            ...
            if( tdc_tmp[0] < -10000){      // after 10.04.2014 the out of range events are the limit of Int_t
                tdc_tmp[0] = -10;
                tdc_tmp[2] = -10;
            }
	    else{
        ...
        if ( fread(&dummyword, sizeof(dummyword), 1, datafile) != 1 ) break; // 0
        if ( fread(&dummyword, sizeof(dummyword), 1, datafile) != 1 ) break; // something
        if ( fread(&dummyword, sizeof(dummyword), 1, datafile) != 1 ) break; // -1
        if ( fread(&time_upper, sizeof(dummyword), 1, datafile) != 1 ) break; // upper 16 clock bits
   //     if ( fread(&time_lower, sizeof(dummyword), 1, datafile) != 1 ) break; // lower 16 clock bits

   //     evt_r.ut = (time_upper << 16) | time_lower;
  ...
  //  Int_t list [1] = {2};
    ...
    for( Int_t i = 0 ; i < 6 ; i++){
      ...
      // gPad->SetLogy();
       ...
       hsp -> GetXaxis()->SetRangeUser(400,1200);
...
void MakeOnePADCPlot2Files(TString rootfile1, TString rootfile2)
{
   ...
   hsp1 = (TH1F*)f1->Get("hp[0]");
   hsp2 = (TH1F*)f2->Get("hp[0]");

   hsp2->SetLineColor(2);
   hsp1 -> GetXaxis()->SetRangeUser(500,1200);

   hsp2->Scale(scaling_factor);

   hsp1->Fit("gaus","","",1000,1100);
   hsp1->GetFunction("gaus")->SetLineColor(4);

   hsp2->Fit("gaus","","",1000,1100);
