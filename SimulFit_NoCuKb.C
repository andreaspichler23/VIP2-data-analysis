...
Double_t funcBg(Double_t *x, Double_t *par){
    ...
    //par[7] = background slope
    
    Double_t back = par[0] + (xx - 7000) * par[7];
    ...
    Double_t niKa1 = par[4]/(sqrt(2*TMath::Pi())*par[6])*TMath::Exp(-((xx-par[5])*(xx-par[5]))/(2*par[6]*par[6]));
    
    Double_t niKa2Gain = par[4] * 0.51;
    Double_t niKa2Mean = par[5] - 17.3;
    
    Double_t niKa2 = niKa2Gain/(sqrt(2*TMath::Pi())*par[6])*TMath::Exp(-((xx-niKa2Mean)*(xx-niKa2Mean))/(2*par[6]*par[6]));

    
    Double_t roiCuFunc = back + cuKa1 + cuKa2  + niKa1 + niKa2;
...
Double_t funcSigBg(Double_t *x, Double_t *par){
    ...
    //par[8] = forbidden gauss gain
    // mean of forbidden transition fixed at 7729 eV
    
    Double_t forbGauss = par[8]/(sqrt(2*TMath::Pi())*par[3])*TMath::Exp(-((xx-7729)*(xx-7729))/(2*par[3]*par[3]));
 
    Double_t roiCuFunc = back + cuKa1 + cuKa2 + niKa1 + niKa2 + forbGauss;
...
struct GlobalChi2 { 
   ...
   double operator() (const double *par) const {
      double p1[8]; // p1 is for background = without current histogram
      ...
      p1[4] = par[4]; // ni ka1 gain ... free
      p1[5] = par[5]; // ni ka1 mean ... fixed
      p1[6] = par[6]; // ni ka1 sigma ... free
      p1[7] = par[7]; // background slope ... common

      double p2[9]; // parameters for the fit with signal with current 
      p2[0] = par[0]; // bg constant ..common parameter
      p2[1] = par[8]; // cu ka1 gain ... free 
      p2[2] = par[9]; // cu ka1 mean ... fixed 
      p2[3] = par[10]; // cu ka1 sigma ... free 
      p2[4] = par[11]; // ni ka1 gain ... free
      p2[5] = par[12]; // ni ka1 mean ... fixed
      p2[6] = par[13]; // ni ka1 sigma ... free
      p2[7] = par[7]; // background slope ... common
      p2[8] = par[14]; // forbidden gauss gain ... free
...
void combinedFit(Int_t reBin) { 
  ...
  Int_t nPar = 15; // does not change in the whole code automatically -> strg + f 15
  Int_t lowerL = 7000;
  Int_t upperL = 8600;
  ...
  Double_t parInit[15] = { 510. * reBin/25 , 310000. * reBin/25 , 8047.78 , 75. , 14900. * reBin/25 , 7478.15 , 72. , -0.02 ,// parameters 0 ... 7 for background and common param
                                             312000. * reBin/25 , 8047.78 , 80. , 12000. * reBin/25 , 7478.15 , 71. , 0. }; // parameters 8 ... 14 including signal

  hSB->Rebin(reBin);
  hB->Rebin(reBin);
  

  TF1 *fB = new TF1("fB", funcBg, lowerL, upperL, 8 );
  TF1 *fSB = new TF1("fSB", funcSigBg, lowerL, upperL, 9 );
...
//  
  ...
  fitter.Config().SetParamsSettings(15,parInit); // i guess this sets the initial parameters
  // fix some parameters 
...
//  fitter.Config().ParSettings(19).Fix();
  ...
  fitter.Config().ParSettings(4).SetName("Ni Ka1 Gain BG");
  fitter.Config().ParSettings(5).SetName("Ni Ka1 Mean");
  fitter.Config().ParSettings(6).SetName("Ni Ka1 Sigma BG");
  fitter.Config().ParSettings(7).SetName("Common Background Slope");
  fitter.Config().ParSettings(8).SetName("Cu Ka1 Gain SIG");
  fitter.Config().ParSettings(9).SetName("Cu Ka1 Mean");
  fitter.Config().ParSettings(10).SetName("Cu Ka1 Sigma SIG");
  fitter.Config().ParSettings(11).SetName("Ni Ka1 Gain SIG");
  fitter.Config().ParSettings(12).SetName("Ni Ka1 Mean");
  fitter.Config().ParSettings(13).SetName("Ni Ka1 Sigma SIG");
  fitter.Config().ParSettings(14).SetName("Forbidden Gauss Gain");
  ...
  fitter.Config().ParSettings(4).SetLimits(10000. * reBin/25,20000. * reBin/25);

  fitter.Config().ParSettings(6).SetLimits(60. ,85. );
  fitter.Config().ParSettings(7).SetLimits(-0.05,0.05);
  fitter.Config().ParSettings(8).SetLimits(200000. * reBin/25,400000. * reBin/25);
  
  fitter.Config().ParSettings(10).SetLimits(75.,85.);

  fitter.Config().ParSettings(11).SetLimits(10000. * reBin/25,15000. * reBin/25);

  fitter.Config().ParSettings(13).SetLimits(65.,75.);
  ...
  fSB->SetParameter(1, result.Parameter(8) ); 
  fSB->SetParameter(2, result.Parameter(9) ); 
  fSB->SetParameter(3, result.Parameter(10) ); 
  fSB->SetParameter(4, result.Parameter(11) ); 
  fSB->SetParameter(5, result.Parameter(12) ); 
  fSB->SetParameter(6, result.Parameter(13) ); 
  fSB->SetParameter(7, result.Parameter(7) ); 
  fSB->SetParameter(8, result.Parameter(14) );
  
  fSB->SetParError(0, result.Error(0)  );
  fSB->SetParError(1, result.Error(8) );
  fSB->SetParError(2, result.Error(9) );
  fSB->SetParError(3, result.Error(10) );
  fSB->SetParError(4, result.Error(11) );
  fSB->SetParError(5, result.Error(12) );
  fSB->SetParError(6, result.Error(13) );
  fSB->SetParError(7, result.Error(7) );
  fSB->SetParError(8, result.Error(14) );
  ...
  Double_t sigCounts = fSB->GetParameter(8);
  Double_t sigError = fSB->GetParError(8);
