...
void MakeTree( TString filelist, TString rootfilename ) 
{
    ...
    FILE  *flist;
    if( (flist = fopen(BIN_PATH_SMI + "/" +filelist, "r")) == NULL ){
        cout << "Cannot open file: " << filelist << endl;
    }
    cout << BIN_PATH_SMI + "/" +filelist << endl;

    char  listline[MAXCHAR];
    char  file_name[MAXCHAR];

    rootfile = ROOT_PATH_SMI + "/" + rootfilename; 
...
//    hannes.open("20151003_SDD.txt");
    ...
    {       
        ...
        sscanf( listline, "%s\n", file_name );  // read name of files from listline and store it under file_name
        cout << file_name << endl;
...
*/
    ...
    TString binary_file = BIN_PATH_SMI + "/" + datafilename; 
 ...
 //   cout << 2 << endl;
    ...
    while(1)
    {
        ...
        // daq rate 
       // if ( fread(&evt_r.evid, sizeof(evt_r.evid), 1, datafile) != 1 ) break;
